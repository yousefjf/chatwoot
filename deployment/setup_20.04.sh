...
CHATWOOT_HUB_URL="https://hub.2.goftina.com/events"
...
Join the community at https://goftina.com/community?utm_source=cwctl
***************************************************************************
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.goftina.com/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
https://goftina.com/docs/deployment/deploy-chatwoot-in-linux-vm?utm_source=cwctl
...
Get help, https://goftina.com/community?utm_source=cwctl
